...
class velocipede(object):
    def __init__(self, gear_ratios, weight, wheel_circumference=5.0,
                 efficiency=0.9):  # gear_ratios is a list of tuples [(p,w)],
    ...
    def __repr__(self):
        return (
            f"Gear is {self.current_gear_ratio} and weight is {round(self.weight, 2)}. \n Located at {round(self.location, 2)}.")

    def change_gears(self,
                     increase):  # expects increase to be an integer. How many gears to shift up if positive, how many down if negative. 0 if no shift.
...
class rider(object):
    ...
    def pedal_energy_and_speed(self, num_rotations):
        ...
        ratio = self.velocipede.current_gear_ratio[1] / self.velocipede.current_gear_ratio[
            0]  # each revolution of the pedals moves the bike this far.
...
class race(object):
    ...
    def __repr__(self):
        ...
        simplified_locations = [(int(racer.velocipede.location / piece_length), racer.name) for racer in
                                self.racers]  # For example, could be [(2,M)] to indicate Mario is on the second piece of track.
        placed_racers = []  # The racers that have been printed.
        return_strings = []
        current_string = []
        while len(placed_racers) < len([racer for racer in self.racers if
                                        racer.velocipede.location < self.track.distance]):  # keep looping until all of the racers that haven't finished have been printed.
            for index in range(25):  # loop through the entire track.
                relevant_markers = [marker for marker in simplified_locations if marker[0] == index and not marker[
                                                                                                                1] in placed_racers]  # Represents the riders that will be placed now.
                if len(relevant_markers) == 0:
                    ...
                    placed_racers.append(relevant_markers[0][
                                             1])  # Add the marker to the placed riders to indicate that this rider has already been printed.
            return_strings.append("".join(current_string) + '\n')
            current_string = []
        return "".join(return_strings)

    def start_race(self,
                   silent=False):  # Does the race. Returns the winner(s). Side-effects: Updates the racers and their velocipedes' locations and speeds.
        while True:
            wins = [racer.handle_timestep(self.track) for racer in self.racers]
            if any(wins):
                return [racer for racer in self.racers if
                        racer.velocipede.location >= self.track.distance]  # Return the list of riders that win.
...
class user_rider(rider):
    def gear_switch_policy(self, track):
        while True:
            try:
                shift = int(input(f"{self.name}, enter gear shift (-1 for down, 1 for up, 0 to stay): "))
                if shift in [-1, 0, 1]:
                    return shift
                print("Invalid input. Enter -1, 0, or 1.")
            except ValueError:
    ...
    def pedal_policy(self, track):
        while True:
            try:
                pedal_count = int(input(f"{self.name}, enter number of pedal rotations (0-10): "))
                if 0 <= pedal_count <= 10:
                    return pedal_count
...
class optimized_rider(rider):
    def __init__(self, name, velocipede_to_ride, weight, max_stamina, regeneration, pedal_energy_cost, silent=False):
        ...
        self.previous_pedal_count = 5  # Start with a moderate pedaling strategy

    def gear_switch_policy(self, track):
        if self.stamina > 0.7 * self.max_stamina:
            return 1  # Shift up if stamina is high
        elif self.stamina < 0.3 * self.max_stamina:
            return -1  # Shift down if stamina is low
        return 0  # Maintain current gear otherwise

    def pedal_policy(self, track):
        if self.stamina > 0.7 * self.max_stamina:
            return min(10, self.previous_pedal_count + 1)  # Increase pedaling if stamina is high
        elif self.stamina < 0.3 * self.max_stamina:
            return max(2, self.previous_pedal_count - 2)  # Reduce pedaling if stamina is low
        return self.previous_pedal_count  # Maintain previous rate otherwise
...
if __name__ == "__main__":
    ...
    mario = rider("Mario", marios_velocipede, 50, 6000, 30,
                  10)  # The parameters are name, velocipede, weight, max_stamina, regeneration, cost per pedal
